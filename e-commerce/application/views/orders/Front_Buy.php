...
class Front_Buy extends Front_Controller {
	...
	public function __construct() {
		...
		parent::__construct();
		if ($this->session->userdata('employeed_logged_in') != '') {
			redirect(base_url());
		}
	 ...
	 * Date 4-11-2017
	 */
	public function place_recharge_order() {
		$total_amount = $this->cart->total();
		$sub_total_amount = $total_amount;

		$recharge_data = $this->session->userdata('rechargevalue');
		...
		if ($employee_wallet_balance >= $total_amount) {
			...
			$emp_details = $this->session->userdata('employeed_logged_in');
	...
	function ordersuccess_send_mail() {
		...
		// $template = ($this->session->userdata('is_recharge')!='' && $this->session->set_userdata('is_recharge') == true) ?  55 : 6 ;
		$template = 7 ;
		$order_id = $this->session->userdata('order_id');
		return $this->email_model->sendMail($email, 'naresh',$template, NULL,$this->session->userdata('order_id'));
	}
