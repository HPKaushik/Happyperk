...
class Aauth {
	...
	public function login($identifier, $pass, $remember = FALSE, $totp_code = NULL) {
		...
		$db_identifier = 'email';
		$query = null;
		$query = $this->aauth_db->where('email', $identifier);
		$query = $this->aauth_db->where('status', 1);
		...
		$query = null;
		$query = $this->aauth_db->where($db_identifier, $identifier);
		$query = $this->aauth_db->where('status', 0);
		$query = $this->aauth_db->get($this->config_vars['users']);
		$row = $query->row();
		// prx($row);
		// if email and pass matches and not banned
		$this->config_vars['use_password_hash'];
		$password = ($this->config_vars['use_password_hash'] == true) ? $pass : $this->hash_password($pass, $row->user_id);
		// $password =  $pass;
		if ( $query->num_rows() != 0 && $this->verify_password($password, $row->password) ) {
			...
			if ( $remember ){
				...
				$this->update_remember($row->user_id, $random_string, $remember_date );
				$cookie = array(
					'name'	 => 'user',
					'value'	 => $row->user_id . "-" . $random_string,
					'expire' => 99*999*999,
					'path'	 => '/',
				);
				$this->CI->input->set_cookie($cookie);
		...
		if ( $this->CI->session->userdata('loggedin') ){ 
			...
			if( ! $this->CI->input->cookie('user', TRUE) ){
				...
				if(!is_numeric( $cookie[0] ) OR strlen($cookie[1]) < 13 ){return FALSE;}
					...
					$query = $this->aauth_db->where('user_id', $cookie[0]);
	...
	public function reset_password($email){
		...
		if( $query->num_rows() > 0 ){
			...
			$data =	 array(
				// 'verification_code' => '',
				'password' => $this->hash_password($pass, $row->user_id)
			);
			...
			$email = $row->email;
			$this->aauth_db->where('user_id', $row->user_id);
			$this->aauth_db->update($this->config_vars['users'] , $data);
			$this->CI->load->library('email');
			if(isset($this->config_vars['email_config']) && is_array($this->config_vars['email_config'])){
				$this->CI->email->initialize($this->config_vars['email_config']);
	...
	public function update_last_login($user_id = FALSE) {
		...
		$data['lastsignedinon'] = date("Y-m-d H:i:s");
	...
	public function get_user_id($email=FALSE) {
		if( ! $email){
			$query = $this->aauth_db->where('user_id', $this->CI->session->userdata('user_id'));
		} else {
			$query = $this->aauth_db->where('email', $email);
		}
		$query = $this->aauth_db->get($this->config_vars['users']);
	...
	public function update_group($group_par, $group_name=FALSE, $definition=FALSE) {
		...
		$this->aauth_db->where('user_id', $group_id);
	...
	// 	$query = $this->aauth_db->where('user_id', $group_id);
	// 	$query = $this->aauth_db->get($this->config_vars['groups']);
	// 	if ($query->num_rows() == 0)
	// 		return FALSE;
	// 	$row = $query->row();
	...
	// 	if ($definition != FALSE)
	// 		$data['definition'] = $definition;
	// 	$this->aauth_db->where('user_id', $perm_id);
 ...
 // 	 * @param int $group_par Group id or name to check
	...
	// 		if ( $this->is_admin( $this->CI->session->userdata('user_id')) OR
	...
	// 	}
	// 	$query = $this->aauth_db->order_by('user_id','DESC');
	...
	// 		return FALSE;
	// 	}
	// 	$query = $this->aauth_db->where('user_id', $pm_id);
	...
	// 			return $this->aauth_db->delete( $this->config_vars['pms'], array('user_id' => $pm_id));			
	// 		}
			
	// 		return $this->aauth_db->update( $this->config_vars['pms'], array('pm_deleted_sender'=>1), array('user_id' => $pm_id));			
	// 	}else if ($user_id == $result->receiver_id){
	// 		if($result->pm_deleted_sender == 1){
	// 			return $this->aauth_db->delete( $this->config_vars['pms'], array('user_id' => $pm_id));			
	// 		}
	// 		return $this->aauth_db->update( $this->config_vars['pms'], array('pm_deleted_receiver'=>1, 'date_read'=>date('Y-m-d H:i:s')), array('user_id' => $pm_id) );
	// 	}
	// }
	// /**
	//  * Cleanup PMs 
	...
	// 	if(!$receiver_id){
	// 		$receiver_id = $this->CI->session->userdata('user_id');
	// 	}
	// 	$query = $this->aauth_db->where('receiver_id', $receiver_id);
	...
	public function error($message = '', $flashdata = FALSE){
		...
		if($flashdata)
		{
			...
			$this->CI->session->set_flashdata('error_msg', $message);
			return true;
		}
